...
RSpec.describe AdminPanel::ProductVariantPolicy do
  subject(:policy) { described_class.new(user, variant) }

  let(:variant) { build(:product_variant) }
  ...
  describe 'inherits from BasePolicy (admin only)' do
